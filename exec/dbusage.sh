...
usage()
{
    echo "
=======================================================================
 A script to count all PostgreSQL databases owned by an user
=======================================================================
...
$0 <da_user>
    ...
    <da_user>  - a directadmin user for which to calculate number of PostgreSQL databases
";
    exit 0;
}
...
dbcount=$(${PSQL_BIN} -tc "SELECT COUNT(datname) as count FROM pg_catalog.pg_database WHERE datname = '${da_user}' OR datname LIKE '${da_user}_%';" 2>/dev/null | xargs); #"
if [ -n "${dbcount}" ]; then
    echo "${dbcount}";
else
    echo 0;
